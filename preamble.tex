...
\usepackage{bytefield}              % Packet for MQTT bits
...
\usepackage{cancel}                 % cancel (cross out) terms in equations nicely
...
\usepackage{lastpage}               % last page for footnote

\usepackage{datetime2}              % custom date format for versioning

\usepackage{svg}                    % allow svg includes

\usepackage{fancybox}               % for ovalbox

\allowdisplaybreaks%                % math stuff allow display breaks in align
...
% --------------------------------------- COLOR DEFINITIONS ---------------------------------------
\definecolor{sectioncolor}{HTML}{FE5D26}
\definecolor{subsectioncolor}{HTML}{F2C078}
...
\definecolor{variableclour}{HTML}{33759E}

\definecolor{mqttflag}{RGB}{255,235,180}

% ----------------------------------- Code and other custom settings for this project
% display code
...
% define length for bitfield in 04_communication.tex MQTT bitfields
\newlength{\mqttbwSixteen}
\newlength{\mqttbwEight}
\newcommand{\bfcol}[2]{%
  \colorbox{#1}{\parbox[c][1.9ex][c]{\dimexpr #2\bitwidth\relax}{\centering\strut}}%
}
\newcommand{\bfcoltxt}[3]{%
  \colorbox{#1}{\parbox[c][1.9ex][c]{\dimexpr #2\bitwidth\relax}{\centering\strut #3}}%
}
...
%   === Text Formatting (font, style and color) ===
    ...
    keywordstyle={[4]\color{variableclour}},
...
% Makro für Pastell-Box
\newtcbox{\pastelbox}[2][]{
  colback=#2!30,
  colframe=#2!60,
  arc=1.5mm,
  boxrule=0pt,
  boxsep=0pt,        % KEIN zusätzlicher Abstand
  left=0.6mm,
  right=0.6mm,
  top=0.2mm,
  bottom=0.2mm,
  nobeforeafter,
  on line,           % wichtig: exakt inline
  #1
}