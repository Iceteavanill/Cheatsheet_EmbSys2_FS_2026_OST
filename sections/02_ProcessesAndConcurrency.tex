...
\begin{itemize}
    ...
    If multiple processes are in the \texttt{Ready} state, the operating system decides, based on its scheduling policy, 
    which one may switch to the \texttt{Running} state.
...
\begin{itemize}
    \item \textbf{CPU utilization:} CPU utilization should be optimized.
        The goal is \qty{100}{\percent} CPU utilization; otherwise, \qty{40}{\percent} \dots \qty{90}{\percent} is typical.
...
Only when the tasks itself yields, a context switch can occur.
...
Interprocess communication and signaling is done via mutexes and semaphores.
They can be taken (p~operation) or released (v~operation).
Typically, the semaphore is taken before entering a critical section and released after.
...
\subsubsection{Mutex}
    ...
    \begin{itemize}
        \item Must be taken and released by the same task. \textrightarrow{} one single tasks is the ``\textbf{owner}''.
    ...
    \begin{itemize}
        \item Can be manipulated from different tasks; any can execute \texttt{give} and \texttt{take}. \textrightarrow{} no ``\textbf{owner}''.
...
Under \emph{priority-based preemptive} scheduling, a low-priority task (LPT) holding a resource blocks a higher-priority task (HPT).
...
\textbf{Queues} are a quasi lock-free instrument: transferring data ownership avoids explicit locks. 
A \textbf{gatekeeper task} with sole ownership of a resource (no locking), or a shared memory pool passed by reference can serve as the sole handler of critical structures.

\vspace{-0.1cm}
\begin{center}
    \includegraphics[width=0.7\columnwidth]{images/PDF/Queues_GatekeeperTask.pdf}
\end{center}
...
\includegraphics[width=\columnwidth]{images/PDF/Deadlock_ProcessSchedule.pdf}
\end{minipage}
