...
\begin{itemize}
    ...
    \item \textbf{Waiting:} The process is waiting for the fulfilment of at least one wait condition.
\end{itemize}
...
\begin{tabular}{lcll}
    ...
    \textbf{Waiting}    & $\Rightarrow$ & \textbf{Ready}    & All waiting conditions got fullfilled\\
...
All modern OS's use preemptive scheduling, this however has the consequence that taks execution is less predictive.
In generall the \emph{save bet} is to use none-preemptive scheduling.
...
\subsubsection{Mutex}
    ...
    A mutex is a locking mechanism used to protect a shared resource.
    \begin{itemize}
...
To remove additional overhead, the current executed task can direcltly be set to the now ready task without waiting for an additional systick:
\begin{lstlisting}
...
\subsubsection{Semaphore}
    % GOAL L04 • You understand the difference between a semaphore and a mutex.
    A semaphore is a signaling mechanism used to notify a waiting task that it can proceed.
    \begin{itemize}
